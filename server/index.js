...
// Initialise GameManager (redis will connect before first use)
const gameManager = new GameManager(io, pubClient);

// Attach once redis connections are ready
Promise.all([pubClient.connect(), subClient.connect()])
  .then(() => {
    console.log('Connected to Redis');
    io.adapter(createAdapter(pubClient, subClient));

    // Handle cross-instance game sync
    subClient.subscribe('gameSync', (message) => {
      try {
        const data = JSON.parse(message);
        if (data.action === 'create' && data.game) {
          if (!gameManager.games.has(data.game.id)) {
            gameManager.games.set(data.game.id, data.game);
          }
        } else if (data.action === 'update' && data.game) {
          gameManager.games.set(data.game.id, data.game);
        } else if (data.action === 'delete' && data.gameId) {
          gameManager.games.delete(data.gameId);
        }
      } catch (err) {
        console.error('Failed to process gameSync message', err);
      }
    });
